...
def train(config, epoch, num_epoch, epoch_iters, base_lr, num_iters,
    ...
    for i_iter, (images, dct_vol, labels, qtable) in enumerate(trainloader):
        images = images.cuda(non_blocking=True)       # [B, 3, H, W]
        dct_vol = dct_vol.cuda(non_blocking=True)     # [B, 21, H, W]
        labels = labels.cuda(non_blocking=True)
        qtable = qtable.cuda(non_blocking=True)       # [B, 21, 8, 8]

        # Concaténer RGB + DCTvol → x: [B, 24, H, W]
        x = torch.cat([images, dct_vol], dim=1)

        losses, _ = model(x, labels, qtable)


        # _ : output of the model (see utils.py)
        loss = losses.mean()
