...
def eval_net_with_curves(model, dataset_name, dataset, save_dir, device='cuda', threshold=0.5):
    loader = DataLoader(dataset, batch_size=2, num_workers=8, shuffle=False)
    model.eval()
...
datasets_info = {
    ...
    "TestingSet":   ("./DocTamperV1-TestingSet", "./pks/DocTamperV1-TestingSet_90.pk", True, range(15000, 16000)),
    "FCD":           ("./DocTamperV1-FCD", "./pks/DocTamperV1-FCD_90.pk", True, range(500, 1500)),
    "SCD":           ("./DocTamperV1-SCD", "./pks/DocTamperV1-SCD_90.pk", True, range(500, 1500)),
}
...
for name, (lmdb_path, record_path, is_train, subset_range) in datasets_info.items():
    ...
    eval_net_with_curves(model, name, subset, save_dir, device=device)
